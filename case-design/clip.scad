...
module batteryClip(width, length, thickness) {
	difference() {
		union() {
			rotate([0,90,0]) cylinder(r=thickness+width/2,h=2*thickness+length,$fn=50,center=true);
			translate([0,0,-thickness/2-width/4]) cube([2*thickness+length,2*thickness+width,thickness+width/2], center=true);
		}
		rotate([0,90,0]) cylinder(r=width/2,h=length,$fn=50,center=true);
...
module 18650holder() {
// 18650 battery (li-ion cell) -with- protection chip!
batteryClip(19,68,2);
}
